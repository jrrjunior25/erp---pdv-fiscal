...
async function testEndpoints() {
  ...
  if (productsRes.data.length > 0) {
    console.log(`   Exemplo: ${productsRes.data[0].name}\n`);
  }
  ...
  if (customersRes.data.length > 0) {
    console.log(`   Exemplo: ${customersRes.data[0].name}\n`);
  }
  ...
  if (suppliersRes.data.length > 0) {
    console.log(`   Exemplo: ${suppliersRes.data[0].name}\n`);
  }
  ...
  console.log('8. GET /api/shifts/history');
  const shiftsHistoryRes = await makeRequest('/api/shifts/history');
  console.log(`   Status: ${shiftsHistoryRes.status}`);
  console.log(`   Total: ${Array.isArray(shiftsHistoryRes.data) ? shiftsHistoryRes.data.length : 0} turnos\n`);

  // 8. Inventory Levels
  console.log('9. GET /api/inventory/levels');
  ...
  // 9. Financials
  console.log('10. GET /api/financials');
  ...
  console.log('11. GET /api/purchasing/orders');
  ...
  if (analyticsRes.data && analyticsRes.status === 200) {
    ...
    console.log(`   Erro ou endpoint não encontrado\n`);
  }
